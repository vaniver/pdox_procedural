...
class Cube:
    ...
    def valid_straits(self, land_cubes, sea_cubes=None):
        buffer = []
        for rot in range(6):
            ...
            kb = self.add(b)
            kab = ka.add(b)
            if sea_cubes is None:
                if ka in land_cubes or kb in land_cubes:
                    continue
            else:
                if ka not in sea_cubes or kb not in sea_cubes:
                    continue
