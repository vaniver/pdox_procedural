...
class HOI4Map(BasicMap):
    ...
    def create_provinces(self, rgb_from_pid, pid_from_cube, coastal, cont_from_pid, terr_from_pid, type_from_pid):
        ...
        # This doesn't use the superclass create_provinces and prov_extra because we need to modify the create_hex_map call.
        ...
        create_hex_map(rgb_from_ijk=rgb_from_ijk, max_x=self.max_x, max_y=self.max_y, mode='RGB', default="black", n_x=self.n_x, n_y=self.n_y).save(os.path.join(self.file_dir, "map", "provinces.bmp"))
    ...
    def create_buildings(self, file_dir, pids_from_rid, coastal):
        ...
        with open(os.path.join(file_dir, "map", "buildings.txt"), 'w', encoding="utf_8_sig") as outf:
...
def create_countries(file_dir, config, region_trees, tech_from_tag, popularities_from_tag = {}, chars_from_tag = {}, max_dnum=75,):
    ...
    for region_tree in region_trees:
        for region in region_tree.all_region_trees():
            if region.tag is not None and region.capital_rid != -1:
                with open(os.path.join(file_dir, "common", "countries", f"{region.title}.txt"), 'w', encoding="utf-8-sig") as outf:
                    outf.write(f"graphical_culture = western_european_gfx\n\tgraphical_culture_2d = western_european_2d\n\ncolor = {{ {region.color} }}")
                country_tag_buffer += f"{region.tag} = \"countries/{region.title}.txt\"\n"
                with open(os.path.join(file_dir, "history", "countries", f"{region.tag}-{region.title}.txt"), 'w', encoding="utf-8-sig") as outf:
    ...
    with open(os.path.join(file_dir, "common", "country_tags", "00_country_tags.txt"), 'w', encoding="utf-8-sig") as outf:
        outf.write(country_tag_buffer)
    assert max_dnum < 100  # We _could_ use additional letters to get more than 100, but... why
    with open(os.path.join(file_dir, "common", "country_tags", "zz_dynamic_tags.txt"), 'w', encoding="utf-8-sig") as outf:
    ...
    for dnum in range(1, max_dnum + 1):
        with open(os.path.join(file_dir, "common", "countries", "D"+str(dnum).rjust(2,"0")+".txt"), 'w', encoding="utf-8-sig") as outf:
...
def create_states(file_dir, config, pids_from_rid, name_from_rid, manpower_from_rid, category_from_rid, tag_from_rid, cores_from_rid, vps_from_rid, buildings_from_rid,):
    ...
    for rid, pids in pids_from_rid.items():
        if name_from_rid[rid][0] == "s":
            continue
        with open(os.path.join(file_dir, "history", "states", f"{rid}-{name_from_rid[rid]}.txt"),'w', encoding="utf-8-sig") as outf:
            outf.write(f"state={{\n\tid={rid}\n\tname=\"{name_from_rid[rid]}\"\n\tmanpower = {manpower_from_rid[rid]}\n\n\tstate_category = {category_from_rid[rid]}\n\n\thistory={{\n\t\towner = {tag_from_rid[rid]}\n")
            outf.write("\n".join([f"\t\tvictory_points = {{ {pid} {vp} }} " for pid, vp in vps_from_rid.items()]))
            outf.write("\n\t\tbuildings = {\n")
...
def create_minimal(file_dir, base_loc, filenames_from_dirs):
    ...
    for dir, file_names in filenames_from_dirs:
        ...
        for file_name in file_names:
            with open(os.path.join(base_loc, *dir, file_name), encoding="utf_8_sig") as inf:
                with open(os.path.join(file_dir, *dir, file_name), 'w', encoding="utf_8_sig") as outf:
                    brackets = 0
...
def create_mod(file_dir, config, pid_from_cube, rgb_from_pid, terr_from_cube, terr_from_pid, rid_from_pid, tag_from_pid, type_from_pid, cont_from_pid, coast_from_cube, pids_from_rid, name_from_rid, river_edges, river_vertices, height_from_vertex, region_trees,):
